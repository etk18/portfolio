...
export const portfolioData = {
    ...
    projects: [
        {
            title: "OnyxBrowser v2.0 — The Agentic Web Engine",
            technologies: ["React", "Electron", "Python", "FastAPI", "ChromaDB", "PyInstaller", "LLMs"],
            year: "2025",
            category: "AI / Desktop",
            flagship: true,
            details: [
                "A natively compiled, AI-first desktop web browser that replaces the standard URL bar with an Agentic Omnibox",
                "Agentic DOM Automation: Extracts token-efficient Spatial DOM Maps and routes them through local LLMs to physically execute native browser interactions (e.g., /click, /summarize)",
                "Semantic Vector Memory: Passively vectorizes web history into a local ChromaDB, allowing natural language search of past browsing sessions via a custom onyx://memory protocol",
                "Standalone Bundling & IPC: Built a custom IPC bridge connecting the Node.js Main Process to a Python/FastAPI backend frozen via PyInstaller into a standalone machine-code binary",
                "Graceful Degradation: Engineered OS-level error handling to catch asynchronous ENOENT spawn failures, allowing the app to fall back to a blazing-fast standard browser if the AI binary is unmatched across OS builds"
            ]
        },
        {
            title: "Spectral Sentinel — AI/ML Deepfake Detection",
            technologies: ["Python", "FastAPI", "Machine Learning", "Computer Vision"],
            year: "2025",
            category: "AI/ML",
            flagship: true,
            details: [
                "A high-performance AI/ML application engineered for the rapid detection and classification of deepfake images",
                "Data Pipeline: Managed and processed the massive ArtiFact dataset, organizing complex image file structures for optimal model training and evaluation",
                "API Architecture: Deployed a robust FastAPI backend to handle high-throughput image processing, routing user uploads through the ML classification pipeline with minimal latency",
                "Detection Engine: Engineered the inference logic to accurately identify synthetic artifacts generated by modern generative AI models"
            ]
        },
        {
